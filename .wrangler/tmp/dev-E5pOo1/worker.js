...
// api/node_modules/hono/dist/request/constants.js
...
// api/node_modules/hono/dist/router/reg-exp-router/matcher.js
var emptyParam = [];
...
// api/node_modules/jose/dist/webapi/lib/buffer_utils.js
...
// api/node_modules/jose/dist/webapi/lib/base64.js
...
// api/node_modules/jose/dist/webapi/util/base64url.js
...
// api/node_modules/jose/dist/webapi/util/errors.js
...
// api/node_modules/jose/dist/webapi/lib/crypto_key.js
...
// api/node_modules/jose/dist/webapi/lib/invalid_key_input.js
...
// api/node_modules/jose/dist/webapi/lib/is_key_like.js
...
// api/node_modules/jose/dist/webapi/lib/is_disjoint.js
...
// api/node_modules/jose/dist/webapi/lib/is_object.js
...
// api/node_modules/jose/dist/webapi/lib/check_key_length.js
...
// api/node_modules/jose/dist/webapi/lib/jwk_to_key.js
...
// api/node_modules/jose/dist/webapi/key/import.js
...
// api/node_modules/jose/dist/webapi/lib/validate_crit.js
...
// api/node_modules/jose/dist/webapi/lib/validate_algorithms.js
...
// api/node_modules/jose/dist/webapi/lib/is_jwk.js
...
// api/node_modules/jose/dist/webapi/lib/normalize_key.js
var cache;
...
// api/node_modules/jose/dist/webapi/lib/check_key_type.js
...
// api/node_modules/jose/dist/webapi/lib/subtle_dsa.js
...
// api/node_modules/jose/dist/webapi/lib/get_sign_verify_key.js
...
// api/node_modules/jose/dist/webapi/lib/verify.js
...
// api/node_modules/jose/dist/webapi/jws/flattened/verify.js
...
// api/node_modules/jose/dist/webapi/jws/compact/verify.js
...
// api/node_modules/jose/dist/webapi/lib/jwt_claims_set.js
...
// api/node_modules/jose/dist/webapi/jwt/verify.js
...
// api/node_modules/jose/dist/webapi/lib/sign.js
...
// api/node_modules/jose/dist/webapi/jws/flattened/sign.js
...
// api/node_modules/jose/dist/webapi/jws/compact/sign.js
...
// api/node_modules/jose/dist/webapi/jwt/sign.js
var SignJWT = class {
  static {
    __name(this, "SignJWT");
  }
...
// api/node_modules/jose/dist/webapi/jwks/local.js
...
// api/node_modules/jose/dist/webapi/jwks/remote.js
...
__export(schema_exports, {
  ...
  questions: () => questions,
  tryoutAttemptItems: () => tryoutAttemptItems,
  tryoutAttempts: () => tryoutAttempts,
  users: () => users
});
...
var questionSubcategories = pgTable("question_subcategories", {
  id: integer("id").primaryKey(),
  categoryId: integer("category_id").references(() => questionCategories.id).notNull(),
  code: text("code").notNull(),
  name: text("name").notNull()
});
var questionTopics = pgTable("question_topics", {
  id: integer("id").primaryKey(),
  code: text("code").notNull(),
  name: text("name").notNull()
});
...
var questionExplanations = pgTable("question_explanations", {
  id: integer("id").primaryKey(),
  questionId: integer("question_id").references(() => questions.id).notNull(),
...
var tryoutAttempts = pgTable("tryout_attempts", {
  id: text("id").primaryKey(),
...
var tryoutAttemptItems = pgTable("tryout_attempt_items", {
...
// api/src/db.ts
ce.fetchFunction = fetch;
var getDb = /* @__PURE__ */ __name(async (env) => {
  if (!env.NEON_DATABASE_URL) {
    throw new Error("NEON_DATABASE_URL is not configured");
  }
  const sql2 = cs(env.NEON_DATABASE_URL);
  return drizzle(sql2, { schema: schema_exports });
}, "getDb");

// api/src/middleware/auth.ts
var getBearer = /* @__PURE__ */ __name((authHeader) => {
  if (!authHeader) return null;
  const m2 = authHeader.match(/^Bearer\s+([^\s]+)$/i);
  return m2 ? m2[1] : null;
}, "getBearer");
var withUserContext = /* @__PURE__ */ __name(async (c, next) => {
...
var requirePremium = /* @__PURE__ */ __name(async (c, next) => {
...
// api/src/index.ts
var resolveCorrectOptionKey = /* @__PURE__ */ __name((opts) => {
...
var getJakartaDayKey = /* @__PURE__ */ __name((nowMs) => {
  const shifted = new Date(nowMs + JAKARTA_OFFSET_MS);
  const y = shifted.getUTCFullYear();
  const m2 = shifted.getUTCMonth() + 1;
  const d2 = shifted.getUTCDate();
  return `${y}-${pad2(m2)}-${pad2(d2)}`;
}, "getJakartaDayKey");
var getJakartaDayIndex = /* @__PURE__ */ __name((nowMs) => Math.floor((nowMs + JAKARTA_OFFSET_MS) / DAY_MS), "getJakartaDayIndex");
var DRILLS_ROTATION_ANCHOR_JAKARTA_UTC_MS = Date.UTC(2026, 3, 1, 0, 0, 0) - JAKARTA_OFFSET_MS;
var DRILLS_ROTATION_ANCHOR_DAY_INDEX = getJakartaDayIndex(DRILLS_ROTATION_ANCHOR_JAKARTA_UTC_MS);
var getJakartaNextMidnightMs = /* @__PURE__ */ __name((nowMs) => {
  const shifted = new Date(nowMs + JAKARTA_OFFSET_MS);
  const y = shifted.getUTCFullYear();
  const m2 = shifted.getUTCMonth();
  const d2 = shifted.getUTCDate();
  return Date.UTC(y, m2, d2 + 1, 0, 0, 0) - JAKARTA_OFFSET_MS;
}, "getJakartaNextMidnightMs");
var getTryoutSecret = /* @__PURE__ */ __name((c) => {
...
var isTryoutPremiumEnabled = /* @__PURE__ */ __name((c) => String(c.env.TRYOUT_PREMIUM_ENABLED || "").toLowerCase() === "true", "isTryoutPremiumEnabled");
var activeQuestionWhere = sql`coalesce(${questions.isActive}, true) = true`;
var categoryWhere = /* @__PURE__ */ __name((code) => {
  const lower = code.toLowerCase();
  const topicMap = { twk: 1, tiu: 2, tkp: 3 };
  const topicId = topicMap[lower];
  return topicId ? eq(questions.topicId, topicId) : sql`false`;
}, "categoryWhere");
var subjectSelect = sql`upper(coalesce(${questionTopics.code}, ${questions.questionType}))`;
app.use(cors());
...
app.get("/db/ping", async (c) => {
  try {
    ...
    return c.json({ ok: true, response: result.rows?.[0]?.response ?? null });
  } catch (error) {
...
app.get("/db/stats", async (c) => {
  try {
    const db = await getDb(c.env);
    const totalRes = await db.select({ count: sql`count(*)` }).from(questions);
    const activeRes = await db.select({ count: sql`count(*)` }).from(questions).where(activeQuestionWhere);
    const codes = ["TWK", "TIU", "TKP"];
    const byCategory = {};
    for (const code of codes) {
      const res = await db.select({ count: sql`count(*)` }).from(questions).where(and(activeQuestionWhere, categoryWhere(code)));
      byCategory[code] = Number(res?.[0]?.count ?? 0);
    }
    return c.json({
      ok: true,
      questions_total: Number(totalRes?.[0]?.count ?? 0),
      questions_active: Number(activeRes?.[0]?.count ?? 0),
...
app.post("/db/import-topic-ids", async (c) => {
  ...
  try {
    ...
    const { questions: questions2 } = body;
    if (!Array.isArray(questions2) || !questions2.length) {
      return c.json({ error: "invalid questions array" }, 400);
    }
    let updated = 0;
    for (const { id, topic_id } of questions2) {
...
app.get("/db/category-audit", async (c) => {
  ...
  try {
    const db = await getDb(c.env);
    const totalActiveRes = await db.select({ count: sql`count(*)` }).from(questions).where(activeQuestionWhere);
    const total_active = Number(totalActiveRes?.[0]?.count ?? 0);
    const withCategoryIdRes = await db.select({ count: sql`count(*)` }).from(questions).where(and(activeQuestionWhere, sql`${questions.categoryId} is not null`));
    const with_category_id = Number(withCategoryIdRes?.[0]?.count ?? 0);
    const without_category_id = Math.max(0, total_active - with_category_id);
    const typeRows = await db.select({ value: sql`lower(coalesce(${questions.questionType}, ''))`, count: sql`count(*)` }).from(questions).where(activeQuestionWhere).groupBy(sql`lower(coalesce(${questions.questionType}, ''))`).orderBy(desc(sql`count(*)`)).limit(50);
    const question_type_values = typeRows.map((r) => String(r.value || "")).filter((v2) => !!v2);
    const mismatchCountRes = await db.select({ count: sql`count(*)` }).from(questions).innerJoin(questionCategories, eq(questions.categoryId, questionCategories.id)).where(
      and(
        activeQuestionWhere,
    ...
    const mismatch_count = Number(mismatchCountRes?.[0]?.count ?? 0);
    const mismatchSamples = await db.select({
      id: questions.id,
      category_code: sql`lower(${questionCategories.code})`,
      question_type: sql`lower(${questions.questionType})`
    }).from(questions).innerJoin(questionCategories, eq(questions.categoryId, questionCategories.id)).where(
      and(
        activeQuestionWhere,
        sql`${questions.categoryId} is not null`,
        sql`${questions.questionType} is not null`,
...
app.get("/drills/daily", async (c) => {
  ...
  const category = requested ?? categories[rotatedIndex];
  try {
    const db = await getDb(c.env);
    const picked = await db.select({
      id: questions.id,
      subject: subjectSelect,
      difficulty: questions.difficulty,
      text: questions.questionText
    }).from(questions).leftJoin(questionTopics, eq(questions.topicId, questionTopics.id)).where(and(activeQuestionWhere, categoryWhere(category))).orderBy(sql`md5((${questions.id})::text || ${dayKey})`).limit(20);
    const drillPicked = picked.length ? picked : await db.select({
      id: questions.id,
      subject: subjectSelect,
      difficulty: questions.difficulty,
      text: questions.questionText
    }).from(questions).leftJoin(questionTopics, eq(questions.topicId, questionTopics.id)).where(activeQuestionWhere).orderBy(sql`md5((${questions.id})::text || ${dayKey} || 'fallback')`).limit(20);
    const ids = drillPicked.map((r) => r.id);
    const opts = ids.length ? await db.select({
      questionId: questionOptions.questionId,
      optionKey: questionOptions.optionKey,
      optionText: questionOptions.optionText,
      isCorrect: questionOptions.isCorrect,
      weight: questionOptions.weight
    }).from(questionOptions).where(inArray(questionOptions.questionId, ids)) : [];
    const grouped = {};
    ...
    const questionsPayload = drillPicked.map((r) => {
      ...
      const correctId = correctByQuestion[questionKey] ?? maxWeightByQuestion[questionKey]?.id ?? (options[0]?.id ?? null);
      const safeSubject = r.subject ?? category;
      return {
        id: r.id,
        subject: safeSubject,
  ...
  try {
    ...
    for (const q of quotas) {
      const rows = await db.select({
        ...
        difficulty: questions.difficulty,
        text: questions.questionText
      }).from(questions).leftJoin(questionTopics, eq(questions.topicId, questionTopics.id)).where(and(activeQuestionWhere, categoryWhere(q.code))).orderBy(sql`md5((${questions.id})::text || ${dayKey})`).limit(q.limit);
      picked.push(...rows);
    }
    if (picked.length < desiredCount) {
      ...
      const extra = await db.select({
        ...
        difficulty: questions.difficulty,
        text: questions.questionText
      }).from(questions).leftJoin(questionTopics, eq(questions.topicId, questionTopics.id)).where(and(activeQuestionWhere, existingIds.length ? notInArray(questions.id, existingIds) : sql`true`)).orderBy(sql`md5((${questions.id})::text || ${dayKey} || 'topup')`).limit(remaining);
      picked.push(...extra);
    }
    if (!picked.length) {
      const fallback = await db.select({
        ...
        difficulty: questions.difficulty,
        text: questions.questionText
      }).from(questions).leftJoin(questionTopics, eq(questions.topicId, questionTopics.id)).where(activeQuestionWhere).orderBy(sql`md5((${questions.id})::text || ${dayKey} || 'fallback')`).limit(desiredCount);
...
app.post("/auth/sync", async (c) => {
  ...
  try {
    const db = await getDb(c.env);
    const existing = await db.select({ id: users.id, isPremium: users.isPremium }).from(users).where(eq(users.id, user.id)).limit(1);
...
app.get("/questions/random", async (c) => {
  ...
  try {
    const db = await getDb(c.env);
    const makeBase = /* @__PURE__ */ __name((whereCondition) => db.select({
      id: questions.id,
      subject: subjectSelect,
      difficulty: questions.difficulty,
      text: questions.questionText
    }).from(questions).leftJoin(questionTopics, eq(questions.topicId, questionTopics.id)).where(whereCondition), "makeBase");
    let rows = category ? await makeBase(and(activeQuestionWhere, categoryWhere(category))).limit(limit) : await makeBase(activeQuestionWhere).limit(limit);
...
app.get("/explanations/:id", requirePremium, async (c) => {
  ...
  try {
    const db = await getDb(c.env);
    const res = await db.select({ explanationText: questionExplanations.explanationText, level: questionExplanations.level }).from(questionExplanations).where(eq(questionExplanations.questionId, questionId)).orderBy(desc(questionExplanations.id)).limit(1);
...
app.get("/tryout/history", requirePremium, async (c) => {
  ...
  try {
    ...
    }).from(tryoutAttempts).where(eq(tryoutAttempts.userId, user.id)).orderBy(desc(tryoutAttempts.createdAt)).limit(100);
...
app.get("/analytics/subtopic-accuracy", requirePremium, async (c) => {
  ...
  try {
    ...
    const rows = await db.select({
      ...
      attempts: sql`count(*)`,
      twkTiuCorrect: sql`sum(case when ${tryoutAttemptItems.isCorrect} = true then 1 else 0 end)`,
      tkpRatioSum: sql`sum(case when ${tryoutAttemptItems.maxWeight} > 0 then (${tryoutAttemptItems.selectedWeight}::float / ${tryoutAttemptItems.maxWeight}::float) else 0 end)`
    }).from(tryoutAttemptItems).leftJoin(questionSubcategories, eq(tryoutAttemptItems.subcategoryId, questionSubcategories.id)).leftJoin(tryoutAttempts, eq(tryoutAttemptItems.attemptId, tryoutAttempts.id)).where(and(eq(tryoutAttempts.userId, user.id), sql`${tryoutAttemptItems.subcategoryId} is not null`)).groupBy(tryoutAttemptItems.categoryCode, tryoutAttemptItems.subcategoryId, questionSubcategories.name);
    const byCategory = {
      TWK: [],
      TIU: [],
      TKP: []
...
app.post("/exam/start", async (c) => {
  ...
  const endsAt = nowMs + durationSeconds * 1e3;
  const seed = crypto.randomUUID();
  try {
    const db = await getDb(c.env);
    const quotas = { TWK: 30, TIU: 35, TKP: 45 };
    const orderedCodes = ["TWK", "TIU", "TKP"];
    const pickedIds = [];
    const topicRows = await db.select({ id: questionTopics.id, code: questionTopics.code }).from(questionTopics).where(inArray(questionTopics.code, orderedCodes));
    const topicIdByCode = /* @__PURE__ */ new Map();
    for (const r of topicRows) topicIdByCode.set(String(r.code), Number(r.id));
    if (topicRows.length === 0) {
      console.warn("TEAM_008 No topics found, falling back to active questions");
      const fallback = await db.select({ id: questions.id }).from(questions).where(activeQuestionWhere).orderBy(sql`md5((${questions.id})::text || ${seed} || 'tryout_fallback')`).limit(110);
      pickedIds.push(...fallback.map((q) => Number(q.id)).filter((n) => Number.isFinite(n)));
    }
    for (const code of orderedCodes) {
      const topicId = topicIdByCode.get(code);
      if (!topicId) continue;
      const quota = quotas[code];
      const subcats = await db.select({ id: questionSubcategories.id }).from(questionSubcategories).innerJoin(questionCategories, eq(questionSubcategories.categoryId, questionCategories.id)).where(eq(questionCategories.topicId, topicId)).orderBy(questionSubcategories.id);
      const subIds = subcats.map((s) => Number(s.id)).filter((n) => Number.isFinite(n));
      if (!subIds.length) {
        const fallback = await db.select({ id: questions.id }).from(questions).where(and(eq(questions.topicId, topicId), eq(questions.isActive, true))).orderBy(sql`md5((${questions.id})::text || ${seed})`).limit(quota);
        pickedIds.push(...fallback.map((q) => Number(q.id)));
        continue;
      }
      ...
      for (const subId of subIds) {
        ...
        if (need <= 0) continue;
        const rows = await db.select({ id: questions.id }).from(questions).where(and(eq(questions.subcategoryId, subId), eq(questions.isActive, true))).orderBy(sql`md5((${questions.id})::text || ${seed} || ${String(subId)})`).limit(need);
        const ids = rows.map((q) => Number(q.id)).filter((n) => Number.isFinite(n));
        pickedIds.push(...ids);
        if (ids.length < need) missing += need - ids.length;
      }
      if (missing > 0) {
        const extra = await db.select({ id: questions.id }).from(questions).where(
          and(
            eq(questions.topicId, topicId),
            eq(questions.isActive, true),
            pickedIds.length ? notInArray(questions.id, pickedIds) : sql`true`
          )
        ).orderBy(sql`md5((${questions.id})::text || ${seed} || 'extra')`).limit(missing);
        pickedIds.push(...extra.map((q) => Number(q.id)).filter((n) => Number.isFinite(n)));
      }
    }
    if (pickedIds.length < 110) {
      ...
      const topup = await db.select({ id: questions.id }).from(questions).where(and(activeQuestionWhere, pickedIds.length ? notInArray(questions.id, pickedIds) : sql`true`)).orderBy(sql`md5((${questions.id})::text || ${seed} || 'tryout_topup')`).limit(remaining);
      pickedIds.push(...topup.map((q) => Number(q.id)).filter((n) => Number.isFinite(n)));
    }
    if (pickedIds.length < 110) {
    ...
    const examId = await new SignJWT({ qids: finalIds, endsAt }).setProtectedHeader({ alg: "HS256" }).setIssuedAt(Math.floor(nowMs / 1e3)).setExpirationTime(Math.floor(endsAt / 1e3)).sign(getTryoutSecret(c));
    return c.json({
      examId,
      type: "SKD",
      startsAt: nowMs,
...
app.get("/exam/:examId/questions", async (c) => {
  ...
  try {
    ...
    const ids = qids.map((n) => Number(n)).filter((n) => Number.isFinite(n));
    ...
    const questionRows = await db.select({
      ...
      difficulty: questions.difficulty,
      text: questions.questionText
    }).from(questions).leftJoin(questionTopics, eq(questions.topicId, questionTopics.id)).where(inArray(questions.id, ids));
    const byId = /* @__PURE__ */ new Map();
    for (const r of questionRows) byId.set(Number(r.id), r);
    ...
    const payloadQuestions = orderedQuestions.map((r) => {
      const questionKey = String(r.id);
      return {
        id: r.id,
        subject: r.subject ?? null,
...
app.post("/exam/:examId/submit", async (c) => {
  ...
  try {
    ...
    if (!ids.length) return c.json({ error: "bad_token" }, 400);
    const db = await getDb(c.env);
    const questionMetaRows = await db.select({
      id: questions.id,
      code: sql`upper(${questionTopics.code})`,
      subcategoryId: questions.subcategoryId,
      subtopicId: questions.subtopicId
    }).from(questions).leftJoin(questionTopics, eq(questions.topicId, questionTopics.id)).where(inArray(questions.id, ids));
    const metaById = /* @__PURE__ */ new Map();
    for (const r of questionMetaRows) {
...
var worker_default = {
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith("/api/") || url.pathname.startsWith("/drills/") || url.pathname.startsWith("/quiz/") || url.pathname.startsWith("/exam/") || url.pathname.startsWith("/auth/") || url.pathname.startsWith("/user/") || url.pathname.startsWith("/questions/") || url.pathname.startsWith("/explanations/") || url.pathname.startsWith("/analytics/") || url.pathname.startsWith("/rank/") || url.pathname.startsWith("/tryout/") || url.pathname.startsWith("/health") || url.pathname.startsWith("/db/")) {
      ...
      return app.fetch(apiRequest, env);
    }
...
// .wrangler/tmp/bundle-8nd4CN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-8nd4CN/middleware-loader.entry.ts
