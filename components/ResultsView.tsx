    ...
    <div className="flex flex-col w-full animate-fade-in pb-20 md:pb-0">
      ...
      <div className="flex flex-col md:flex-row border-b border-black">
          ...
          <div className="md:w-1/2 p-12 bg-brand-lime border-b md:border-b-0 md:border-r border-black flex flex-col justify-center items-center text-center">
              <span className="font-bold text-xs uppercase tracking-[0.2em] mb-4">Session Result</span>
              <h1 className="text-9xl font-black mb-2 leading-none tracking-tighter">
                  {Math.round((correctAnswers / totalQuestions) * 100)}%
              </h1>
              <p className="font-bold text-xl mb-6">You answered {correctAnswers} out of {totalQuestions} correctly.</p>
              <div className="inline-block px-4 py-2 border border-black bg-white rounded-full text-xs font-black uppercase">
                  Top {100 - session.percentile}% Percentile
              </div>
          </div>

          {/* Right: Actions */}
          <div className="md:w-1/2 bg-white p-8 flex flex-col justify-center items-start space-y-4">
              <h2 className="text-3xl font-black uppercase mb-2">Next Steps</h2>
              <p className="text-gray-600 mb-6 max-w-sm">Great effort. Review your mistakes below or save this session to your profile.</p>
      ...
      <div className="bg-bg">
          <div className="p-4 border-b border-black flex items-center gap-2 bg-gray-50">
              ...
              <h3 className="font-black text-sm uppercase tracking-wider">Detailed Review</h3>
          </div>
                  ...
                  {isOpen && (
                    <div className="p-6 pt-0 pl-20 max-w-3xl">
                      <div className="p-6 bg-brand-cream border border-black text-sm space-y-4">
                          <div>
                              <span className="font-black uppercase text-xs block mb-1 text-gray-500">Question</span>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div className={`p-3 border border-black ${isCorrect ? 'bg-brand-lime' : 'bg-brand-pink/20'}`}>
                                  <span className="text-[10px] font-black uppercase mb-1 block">Your Answer</span>
                                  <span className="font-bold">{q.options.find(o => o.id === session.answers[q.id])?.text || 'Skipped'}</span>
                              </div>
                              <div className="p-3 border border-black bg-white">
                                  <span className="text-[10px] font-black uppercase mb-1 block">Correct Answer</span>
                                  <span className="font-bold">{q.options.find(o => o.id === q.correct_option_id)?.text}</span>
                              </div>
                          </div>

                          <div>
                              <span className="font-black uppercase text-xs block mb-1 text-gray-500">Explanation</span>
