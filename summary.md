...
| 2 | Fix `renderMarkdown`: escape `<>`, bold, numbered list, `\n` → `<p>`. Regex renderer (`ponytail`). | ✅ (commit `a55e5b2`) — hapus `prose*` + reuse shared renderMarkdown utk fallback. |
| 3 | Wiring: `RoadmapView` pass `theme.name`; `App.tsx` state + `key=` remount (ganti reset-effect). | ✅ (commit `b01175d`) |
